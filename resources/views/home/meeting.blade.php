...
@section('content')
    <main dir="rtl" class="flex flex-col items-center justify-between p-4 md:p-24 min-h-screen"
        ...
        <div className="flex flex-col md:flex-row justify-center items-center space-y-4 md:space-y-0 md:space-x-20 mt-8">
            <button className="text-white rounded-md bg-gradient-to-r from-orange-400  text-4xl w-96 h-24">خوولی
                ڕووبەڕوو</button>
        </div>
        <form class='mx-20' wire:submit.prevent="postMeeting">
            <div>
                @error('userData')
                    <span class="error">{{ $message }}</span>
                @enderror
            </div>
            <x-input-form icon="/phone.svg" name="email" placeholder="Your email address" type="email"
                maxlength="255" />
        </form>
