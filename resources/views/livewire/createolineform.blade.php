<div>
    <form wire:submit="save">
        <div class="relative mt-5 w-full">
            ...
            <input wire:model="userData.name" type="text" id="name" name="name"
        ...
        <div class="relative mt-5 w-full">
            ...
            <input wire:model="userData.phone" type="text" id="phone" name="phone"
                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full ps-10 p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
        ...
        <div class="mt-5">
            ...
            <select wire:model="contentId" id="contentId"
                ...
                @foreach ($contentTypes as $contentType)
                    <option class="text-gray-500" value="{{ $contentType->contentId }}">{{ $contentType->amount }}$
        ...
        <div class="mt-5">
            ...
            <select wire:model="certificateId" id="certificateId"
                ...
                @foreach ($certificates as $certificate)
                    <option class="text-gray-500" value="{{ $certificate->certificateId }}">{{ $certificate->name }}
                    </option>
                @endforeach
            </select>
        </div>
        ...
        <div class="mt-5">
            <select wire:model="socialId" id="socialId"
                ...
                @foreach ($socialUses as $socialUse)
                    <option value="{{ $socialUse->socialId }}" class="text-gray-700">{{ $socialUse->name }}</option>
                @endforeach
            </select>
        </div>
        @error('socialId')
